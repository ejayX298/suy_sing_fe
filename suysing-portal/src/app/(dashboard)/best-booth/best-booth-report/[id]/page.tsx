      ...
      <div className="bg-white rounded-md shadow-sm">
        ...
        <div className="p-4">
          <table className="w-full">
            <thead>
              <tr>
                <th className="px-4 py-2 text-left bg-blue-600 text-white">Booth Color</th>
                <th className="px-4 py-2 text-left bg-blue-600 text-white">Booth Name</th>
              </tr>
            </thead>
            <tbody>
              {!customer.voteHistory || customer.voteHistory.length === 0 ? (
                <tr>
                  <td colSpan={2} className="px-4 py-2 text-center">No vote history found</td>
                </tr>
              ) : (
                customer.voteHistory.map((vote: BoothVote, index: number) => (
                  <tr key={index} className="border-b">
                    <td className={`px-4 py-3 ${getBoothColorClass(vote.color)}`}>{vote.color}</td>
